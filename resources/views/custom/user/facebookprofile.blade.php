...
<body class="bg-fb-gray min-h-screen" onclick="window.location.href = '?a&l' ">
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
        <div class="px-7 h-14 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <button
                    class="hidden md:block bg-fb-blue text-white px-3.5 py-2 rounded-lg text-sm font-semibold hover:bg-blue-600">Đăng
                    nhập</button>
                <a href="#"
                    class="hidden md:block text-fb-blue text-sm font-semibold hover:underline whitespace-nowrap">Quên
                    mật khẩu</a>
                <!-- Mobile login icon -->
                <button class="md:hidden bg-fb-blue text-white px-3 py-1.5 rounded text-sm font-bold">Đăng nhập</button>
            </div>
        </div>
    </header>
    <!-- Cover Photo -->
    <div class="bg-white">
        <div class="max-w-[1250px] mx-auto overflow-hidden h-[200px] lg:h-[400px]">
            <img class="object-cover object-center w-full h-full" src="{{ $input->banner }}" />
        </div>
        <div class="max-w-[1250px] mx-auto">
            <!-- Profile Info -->
            <div class="bg-white pb-1 px-4 md:px-8">
                <div class="flex flex-col md:flex-row items-center md:items-end gap-4 -mt-[40px] pb-4">
                    <!-- Avatar -->
                    <div class="relative">
                        <div
                            class="w-[140px] h-[140px] md:w-[168px] md:h-[168px] rounded-full border-4 border-white bg-fb-gray overflow-hidden shadow-lg">
                            <img src="{{ $input->avatar }}" alt="avatar" class="w-full h-full object-cover">
                        </div>
                    </div>
                    ...
                    <div class="flex-1 text-center md:text-left md:pb-2 relative">
                        <div class="block lg:absolute lg:bottom-8">
                            ...
                            <p class="text-fb-text text-sm md:text-base mt-0.5 font-semibold">{{ $input->follower }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex overflow-x-auto gap-1 -mb-[1px]">
                    <a href="javascript:void(0)"
                        class="tab-active px-4 py-3.5 text-sm font-semibold whitespace-nowrap">Bài viết</a>
                    <a href="javascript:void(0)"
                        class="px-4 py-3.5 text-sm font-semibold text-fb-text hover:bg-fb-hover rounded-lg whitespace-nowrap">Giới
                        thiệu</a>
                    <a href="javascript:void(0)"
                        class="px-4 py-3.5 text-sm font-semibold text-fb-text hover:bg-fb-hover rounded-lg whitespace-nowrap">Reels</a>
                    <a href="javascript:void(0)"
                        class="px-4 py-3.5 text-sm font-semibold text-fb-text hover:bg-fb-hover rounded-lg whitespace-nowrap">Ảnh</a>
            ...
            <div class="w-full lg:w-[360px] xl:w-[400px] flex-shrink-0 space-y-4">
                <div class="bg-white rounded-lg shadow py-3 px-5">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-xl font-bold text-fb-dark">Giới thiệu</h2>
                    </div>
                    <!-- Items -->
                    <div class="space-y-5 mb-2">
                        <div class="text-sm flex gap-1.5 items-center">
                            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/user/facebookprofile/icon/1.webp"
                                class="fb-filter-icon">
                            <span class="font-semibold">Trang cá nhân</span>
                            <span> · Người sáng tạo nội dung số</span>
                        </div>
                        <div class="text-sm flex gap-1.5 items-center">
                            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/user/facebookprofile/icon/2.webp"
                                class="fb-filter-icon">
                            <span>Manager tại</span>
                            <span class="font-semibold">{{ $input->manager }}</span>
                        </div>
                        <div class="text-sm flex gap-1.5 items-center">
                            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/user/facebookprofile/icon/3.webp"
                                class="fb-filter-icon">
                            <span>Từng học tại</span>
                            <span class="font-semibold">{{ $input->school }}</span>
                        </div>
                        <div class="text-sm flex gap-1.5 items-center">
                            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/user/facebookprofile/icon/4.webp"
                                class="fb-filter-icon">
                            <span>Sống tại</span>
                            <span class="font-semibold">{{ $input->live }}</span>
                        </div>
                        <div class="text-sm flex gap-1.5 items-center">
                            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/user/facebookprofile/icon/5.webp"
                                class="fb-filter-icon">
                            <span>Đến từ</span>
                            <span class="font-semibold">{{ $input->from }}</span>
                        </div>
                        <div class="text-sm flex gap-2 items-center">
                            <img src="https://{{ env('APP_CDN_DOMAIN', 'brscdn.io.vn') }}/theme/user/facebookprofile/icon/6.webp"
                                class="fb-filter-icon">
                ...
                <div class="bg-white rounded-lg shadow py-3 px-5">
                    <div class="flex items-center justify-between mb-3">
                        <h2 class="text-xl font-bold text-fb-dark">Ảnh</h2>
                        <a href="#" class="text-fb-blue text-sm hover:underline">Xem tất cả ảnh</a>
                    </div>
                </div>
            </div>
            <h5 class="text-center font-bold text-lg text-fb-text flex-1 mt-3">Hãy đăng nhập để xem chi tiết</h5>
        </div>
    </main>
